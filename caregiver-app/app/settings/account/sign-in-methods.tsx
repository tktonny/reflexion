...
function showUnavailable(method: 'Google' | 'Apple', continueWithEmail: () => void) {
...
export default function Methods() {
  ...
  return <ScreenLayout contentContainerStyle={styles.content}><AppHeader title="Sign-in methods" onBack={() => router.back()} /><Text accessibilityRole="header" style={styles.title}>Sign-in methods</Text><Text style={styles.copy}>Email and phone sign-in use the account details saved in Reflexion. Google and Apple sign-in are not available during the current pilot.</Text><View style={styles.group}><SettingsRow icon="mail" label="Email and password" value="Configured" /><SettingsRow icon="smartphone" label="Phone sign-in" value="Use the saved phone number" /><SettingsRow icon="chrome" label="Google" value="Unavailable during pilot" onPress={() => showUnavailable('Google', continueWithEmail)} /><SettingsRow icon="command" label="Apple" value="Unavailable during pilot" onPress={() => showUnavailable('Apple', continueWithEmail)} /></View></ScreenLayout>;
}
