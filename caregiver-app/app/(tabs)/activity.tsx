...
function messageEvent(name: string, patientId: string, message: V1FamilyMessage): Event { const at = Date.parse(message.createdAt); const status = ({ scheduled: 'scheduled', queued: 'queued', delivered: 'delivered to the device', opened: 'delivered to the device and viewed' })[message.state]; return { id: `message-${message.messageId}`, patientId, title: 'Family message', detail: `A message for ${name} is ${status}.`, time: formatTime(message.createdAt), at, kind: 'chat', icon: 'send' }; }
