...
import { AppHeader, PrimaryButton, ScreenLayout, SelectionButton } from '../../../src/components/AppUI';
...
export default function MessageComposerScreen() {
  ...
  return <ScreenLayout><AppHeader title="New message" onBack={() => router.back()} /><Text accessibilityRole="header" style={styles.title}>Send a message</Text><Text style={styles.subtitle}>A notification appears on the paired Mirror. The message itself remains private until it is opened.</Text><MessageTypePicker selected="text" onSelect={(type) => { if (type === 'photo') router.push(`/chat/${id}/photo`); if (type === 'voice') router.push(`/chat/${id}/voice`); }} /><Field label="Message" multiline onChangeText={setMessage} placeholder="Write something warm and familiar" value={message} /><Text style={styles.label}>Quick reactions</Text><View style={styles.emojiRow}>{['❤️', '👍', '😊', '😂', '🙏'].map((emoji) => <MotionPressable key={emoji} accessibilityLabel={`Add ${emoji}`} accessibilityRole="button" haptic="selection" onPress={() => setMessage((current) => `${current}${current ? ' ' : ''}${emoji}`)} style={styles.emojiButton}><Text style={styles.emoji}>{emoji}</Text></MotionPressable>)}</View><Text style={styles.label}>When should it arrive?</Text><View style={styles.schedule}><SelectionButton label="Send now" onPress={() => setSchedule('now')} selected={schedule === 'now'} /><SelectionButton label="Choose date and time" onPress={() => setSchedule('specific-date-and-time')} selected={schedule === 'specific-date-and-time'} /></View>{schedule === 'specific-date-and-time' ? <Field label="Delivery date and time" autoCapitalize="none" onChangeText={setScheduledFor} placeholder="2026-08-04T09:00" value={scheduledFor} /> : <Text style={styles.scheduleNote}>It will be queued for delivery now and shown when the paired Mirror checks for messages.</Text>}<PrimaryButton label="Preview message" onPress={preview} /></ScreenLayout>;
}
const styles = StyleSheet.create({ title: { color: colors.text.primary, fontFamily: fontFamily.display, fontSize: fontSize.title, fontWeight: '500', marginTop: spacing.lg }, subtitle: { color: colors.text.secondary, flexShrink: 1, fontSize: fontSize.body, lineHeight: 22 }, label: { color: colors.text.primary, fontSize: fontSize.body, fontWeight: '700', marginTop: spacing.md }, schedule: { gap: spacing.sm }, scheduleNote: { color: colors.text.secondary, flexShrink: 1, fontSize: fontSize.caption, lineHeight: 18 }, emojiRow: { flexDirection: 'row', gap: spacing.sm, minWidth: 0 }, emojiButton: { alignItems: 'center', backgroundColor: '#F2F8F6', borderColor: '#D4E6DF', borderRadius: radius.md, borderWidth: 1, flex: 1, justifyContent: 'center', minHeight: 44, minWidth: 0 }, emoji: { fontSize: 22 } });
