...
> **2026-07-22 production update:** sections describing a self-contained APK with a compiled-in
> Qwen key are obsolete and must not be followed. Production builds use
> `EXPO_PUBLIC_API_BASE=https://reflexion.production.tktonny.top` and obtain a short-lived realtime
> ticket from `/api/v1/sessions/:sessionId/realtime-tickets`. Server credentials belong only in
> `.env.server.local`; see `docs/ANDROID_BUILD.md` for the current release procedure.
...
### B2. 安装 APK（当前生产链路）
APK 必须通过统一后端认证，并使用服务端签发的短期 realtime ticket；长期 Qwen key 不得进入 Expo 环境变量或 APK。`eas.json` 的 `release-apk` profile 已指向生产 API。
```bash
cd REFLEXION/mirror-app
npx eas-cli login
...
## 已验证(无需真机的冒烟)
```bash
node --env-file=.env.server.local server/smoke.mjs            # v1 relay→Qwen
node --env-file=.env.server.local server/smoke-turnbased.mjs  # v2 chat/tts/asr
node --env-file=.env.server.local server/smoke-turnloop.mjs   # v2 4阶段编排
node --env-file=.env.server.local server/smoke-direct-ws.mjs  # v3 token→直连WS
node --env-file=.env.server.local server/smoke-assess.mjs     # 判断结果(健康 vs 受损)
node server/smoke-hwcheck.mjs                     # 硬件自检决策表(7 组合全 PASS)
node --env-file=.env.server.local server/smoke-vision.mjs      # 多模态视觉筛查(qwen-vl-max 收图+返回完整判断)
```

## 视频输入（开发研究夹具，不属于生产每日会话）
`/realtime-test` 开发页保留了摄像头实验：界面可显示实时预览(`src/components/MirrorCameraPanel.tsx`)，
会话进行中每 ~8 秒采样一帧，判断卡片多出「视觉观察」一栏。生产 `conversation.tsx` 已移除自动相机权限请求和抽帧；不得把这个研究夹具描述成当前 MVP 的日常功能。

**两阶段(打分模型不看画面 —— 结构上保证"分类只依据对话"):**
1. 阶段一(视觉,`qwen-vl-max`):帧 → 仅产出 `visual_observations`(参与度/情绪),不产出分类/分数。
2. 阶段二(文本,`qwen-plus`):转写 +(视觉观察作为辅助)→ 风险分类。帧不进入打分调用。

其它防护(均来自对抗式审查 + 已修):
- 每帧下采样到 **512px JPEG**(`expo-image-manipulator`)—— 否则 web 是全分辨率 PNG、原生是全传感器分辨率,6 帧会撑爆请求。
- **降级**:阶段一失败 → 仅凭转写照常出分类;帧让服务端请求失败 → 自动重试纯文本(主判断绝不因视频丢失)。
- **不泄露 PII**:服务端可达时绝不把人脸帧走客户端直连;仅在无后端(独立 APK)才客户端直连。
- 所有评估请求都有 **45s 超时**,UI 不会永久卡在"评估中"。
- 帧数上限 6(客户端 + 服务端各 cap)。
- web:`npm run web` → `/realtime-test` → 允许摄像头;原生同一套 `CameraView`(dev build 生效)。
- 验证:`npm run smoke:vision`(两阶段:视觉观察 → 文本分类,真机 key 实测 PASS)。
