...
> **Production note (2026-07-22):** all Android modes authenticate through
> `https://reflexion.production.tktonny.top`. Provider account keys are server-only and must never
> be placed in `EXPO_PUBLIC_*`; the Mirror requests a short-lived realtime ticket from `/api/v1`.
...
编辑 `.env` 的 `EXPO_PUBLIC_CONVERSATION_MODE`。生产 `http/ws` 均通过统一后端获取短期凭证；客户端不保存长期 Qwen key。
...
## 无需真机就能跑的验证(已全部 PASS)
```bash
node --env-file=.env.server.local server/smoke.mjs            # v1: 中继→Qwen
node --env-file=.env.server.local server/smoke-turnbased.mjs  # v2: chat/tts/asr 三段
node --env-file=.env.server.local server/smoke-turnloop.mjs   # v2: 4 阶段编排大脑
node --env-file=.env.server.local server/smoke-token.mjs      # token 签发
node --env-file=.env.server.local server/smoke-direct-ws.mjs  # v3: 临时token→直连WS→编排→audio
```
