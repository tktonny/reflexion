...
import { getDeviceCredential, getDevicePairingIdentity } from '../storage/deviceCredentials'
...
async function checkDeviceIdentity(): Promise<HardwareCheck> {
  const label = '设备身份 / 凭证'
  try {
    const [identity, credential] = await Promise.all([getDevicePairingIdentity(), getDeviceCredential()])
    if (credential) {
      // Paired. Legacy builds may still carry a bootstrap token; flag a mismatch if one is present.
    ...
    return identity.source === 'local'
      ? { key: 'identity', label, status: 'ok', detail: `待配对(本机身份 …${identity.deviceId.slice(-6)})` }
      : { key: 'identity', label, status: 'ok', detail: `待配对(设备 …${identity.deviceId.slice(-6)})` }
  } catch {
