...
export function useTurnBasedConversation(options: ConversationOptions = {}): ConversationApi {
  ...
  const startConversation = useCallback(async () => {
    ...
    try {
      ...
      const opening = openingMessageForLanguage(language, dailyPlan.patientName, greetingPeriod)
  ...
  const runTurn = useCallback(async () => {
    ...
    try {
      ...
      const reply = persona === 'companion' && userGoodbyeRequested
