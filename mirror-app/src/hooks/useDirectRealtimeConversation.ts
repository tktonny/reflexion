...
export function useDirectRealtimeConversation(options: Options = {}): ConversationApi {
  ...
  const transition = useCallback((event: TurnTakingEvent) => {
    ...
    dbg.log(`turn ${event.type}->${next.phase} mic=${next.captureMuted ? 'muted' : 'open'}`)
  ...
  const completeConversation = useCallback(() => {
    ...
    dbg.log(`complete goodbye turns=${turnCountRef.current}`)
  ...
  const failClosed = useCallback((reason: string) => {
    if (endedRef.current) return
    dbg.log(`failClosed turns=${turnCountRef.current} reason=${String(reason).slice(0, 240)}`)
    endedRef.current = true
  ...
  const resumeListening = useCallback(() => {
    dbg.log(`resumeListening ptt=${pushToTalk} manual=${manualCloseRequestedRef.current} wrapping=${wrappingUpRef.current}`)
    ...
    drainTimerRef.current = setTimeout(() => {
      drainTimerRef.current = null
      drainInProgressRef.current = false
      dbg.log(`mic reopen timer manual=${manualCloseRequestedRef.current} wrapping=${wrappingUpRef.current} audio=${Boolean(audioRef.current)}`)
  ...
  const handlePlaybackComplete = useCallback((closingResponse: boolean) => {
    ...
    dbg.log(`playback complete closing=${closingResponse} goodbye=${goodbyeDetectedRef.current} action=${action}`)
  ...
  const handleMessage = useCallback(
    (payload: any) => {
      ...
      if (type === 'session.updated') {
        dbg.log(`session.updated resume=${resumingRef.current} opening=${openingRequestedRef.current} persona=${persona}`)
        if (resumingRef.current) {
          ...
          if (persona === 'screening') {
            ...
            void playDeterministicResponse(
              openingTextForLanguage(voiceRef.current.languageKey, dailyPlan.patientName, greetingPeriodForDate()),
              false,
            )
          } else {
            responseRequestedRef.current = true
      ...
      if (type === 'response.created') {
        ...
        hadResponseRef.current = true
        dbg.log(`response.created closing=${closingRef.current} turns=${turnCountRef.current}`)
  ...
  const startConversation = useCallback(async (opts?: { resume?: boolean }) => {
    ...
    try {
      ...
      socket.onerror = () => {
        ...
        dbg.log(`ws error opened=${openedRef.current} hadResponse=${hadResponseRef.current} turns=${turnCountRef.current}`)
      ...
      socket.onclose = () => {
        ...
        if (!runtimeLease.isCurrent() || startAttemptRef.current !== startAttempt) return
        dbg.log(`ws close opened=${openedRef.current} hadResponse=${hadResponseRef.current} ended=${endedRef.current} turns=${turnCountRef.current}`)
