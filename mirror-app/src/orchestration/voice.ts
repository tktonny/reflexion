...
export function voiceProfileFromRecentSignals(args: {
  ...
  if (['minnan', 'cantonese'].includes(latest.languageKey) && latest.confidence >= 0.55) return voiceProfileForLanguageKey(latest.languageKey, latest.source)
  if (latest.languageKey === 'mandarin' && latest.confidence >= 0.6) return voiceProfileForLanguageKey(latest.languageKey, latest.source)
  if (latest.confidence >= 0.65) return voiceProfileForLanguageKey(latest.languageKey, latest.source)
  ...
  const hintedKey = normalizeLanguageKey(languageHint)
  if (latest.confidence >= 0.5) return voiceProfileForLanguageKey(latest.languageKey, latest.source)
  return null
}
