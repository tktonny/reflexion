...
test('does not repeat future prompts already answered in one multi-topic turn', () => {
  const flow = createSessionCheckinFlow('english', createDailyConversationPlan({ reminiscenceWeekdays: [] }))
  assert.equal(flow.current()!.questionId, 'yesterday_dinner')
  assert.equal(flow.recordAnswer('I had dinner and slept well'), 'answered')
  assert.equal(flow.current()!.questionId, 'today_plan', 'sleep is covered without repeating it')
  assert.equal(flow.recordAnswer('I am planning to visit Mei today'), 'answered')
  assert.equal(flow.current(), null, 'planning and visiting were covered without repetitive prompts')
})

test('silence is insufficient, while a short complete answer advances', () => {
  const flow = createSessionCheckinFlow('english', createDailyConversationPlan({ reminiscenceWeekdays: [] }))
  const first = flow.current()!
  assert.equal(flow.recordAnswer(''), 'insufficient') // no speech
  assert.equal(flow.current()!.questionId, first.questionId, 'stays on the same question')
  assert.equal(flow.recordAnswer('okay'), 'answered')
