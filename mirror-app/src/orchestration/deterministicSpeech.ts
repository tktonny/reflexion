...
export function greetingPeriodForDate(value: Date = new Date()): GreetingPeriod {
...
const BASE_QUESTIONS: Partial<Record<LanguageKey, string[]>> = {
  english: [
    "I was wondering — what did you have for dinner last night?",
    "And how was your sleep? Did you rest well?",
    "So what are you thinking of doing today?",
    "And is anyone coming to see you this week?",
  ],
  mandarin: ['我想问一下，你昨晚吃了什么呀？', '那你睡得怎么样，休息得好吗？', '今天有什么打算呢？', '这周有人来看你吗？'],
  cantonese: ['我想問下，你尋晚食咗啲咩呀？', '噉你尋晚瞓得好唔好呀？', '你今日有咩打算呀？', '今個星期會唔會有人嚟探你呀？'],
  minnan: ['我想欲問你，昨昏暗頓食啥物？', '你昨暗睏了有舒服無？', '今仔日拍算欲做啥物？', '這禮拜有人欲來看你無？'],
  malay: ['Saya tertanya-tanya, apa yang anda makan untuk makan malam semalam?', 'Bagaimana tidur anda malam tadi? Tidur lena?', 'Apa yang anda rancang untuk lakukan hari ini?', 'Ada sesiapa yang akan melawat anda minggu ini?'],
  tamil: ['நான் யோசித்துக் கொண்டிருந்தேன் — நேற்று இரவு உணவிற்கு என்ன சாப்பிட்டீர்கள்?', 'உங்கள் தூக்கம் எப்படி இருந்தது? நன்றாக ஓய்வெடுத்தீர்களா?', 'இன்று என்ன செய்யத் திட்டமிட்டுள்ளீர்கள்?', 'இந்த வாரம் யாராவது உங்களைப் பார்க்க வருகிறார்களா?'],
}
...
export function openingTextForLanguage(
  ...
  switch (language) {
    case 'mandarin': {
      const greeting = period === 'morning' ? '早上好' : period === 'afternoon' ? '下午好' : '晚上好'
      return `${name}，${greeting}！这是你今天的健康检查对话。你今天感觉怎么样？`
    }
    case 'cantonese': {
      const greeting = period === 'morning' ? '早晨' : period === 'afternoon' ? '午安' : '晚安'
      return `${name}，${greeting}！呢個係你今日嘅健康檢查對話。你今日感覺點呀？`
    }
    case 'minnan': {
      const greeting = period === 'morning' ? '早安' : period === 'afternoon' ? '下晡好' : '暗安'
      return `${name}，${greeting}！這是你今仔日的健康檢查對話。你今仔日感覺按怎？`
    }
    case 'malay': {
      const greeting = period === 'morning' ? 'Selamat pagi' : period === 'afternoon' ? 'Selamat petang' : 'Selamat malam'
      return `${greeting} ${name}! Ini adalah pemeriksaan kesihatan harian anda. Bagaimana perasaan anda hari ini?`
    }
    case 'tamil': {
      const greeting = period === 'morning' ? 'காலை வணக்கம்' : period === 'afternoon' ? 'மதிய வணக்கம்' : 'மாலை வணக்கம்'
      return `${greeting} ${name}! இது உங்கள் தினசரி உடல்நல பரிசோதனை. இன்று எப்படி உணர்கிறீர்கள்?`
    }
    default: return `Good ${period} ${name}, it's lovely to see you. How are you feeling today?`
  }
}
...
const ACKNOWLEDGEMENTS: Partial<Record<LanguageKey, string[]>> = {
  english: ['Oh that\'s nice to hear.', 'I love that.', 'Thanks for sharing that with me.', 'That\'s really lovely.'],
  mandarin: ['哦，真好。', '我很喜欢听你这么说。', '谢谢你跟我分享。', '真有意思。'],
  cantonese: ['哦，咁好呀。', '我好鍾意聽你咁講。', '多謝你同我分享。', '真係幾好喎。'],
  minnan: ['哦，真好。', '我真愛聽你按呢講。', '多謝你共我分享。', '真趣味。'],
  malay: ['Oh, itu menarik.', 'Saya suka mendengarnya.', 'Terima kasih kerana berkongsi.', 'Itu sangat baik.'],
  tamil: ['ஓ, மிகவும் நன்றாக இருக்கிறது.', 'நான் அதைக் கேட்க விரும்புகிறேன்.', 'பகிர்ந்ததற்கு நன்றி.', 'அது மிகவும் அருமை.'],
}
