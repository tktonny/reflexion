...
import { greetingPeriodForDate } from './deterministicSpeech'
...
export const REALTIME = {
  ...
  vadThreshold: 0.5,
  // Balanced for real-world use: long enough for natural pauses, short enough to respond promptly.
...
function currentLocalTimeLine(language: string, now = new Date()): string {
  try {
    const locale = /mandarin|chinese|zh|粤|cantonese|闽|minnan/i.test(language) ? 'zh-CN' : 'en-US'
    return new Intl.DateTimeFormat(locale, { dateStyle: 'full', timeStyle: 'short' }).format(now)
  } catch {
    return now.toString()
  }
}
