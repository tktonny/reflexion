...
export type CheckinQuestion = {
  questionId: string
  /** Semantic prompt identity used to avoid asking for a topic already covered in a multi-topic answer. */
  responseTopic: 'dinner' | 'sleep' | 'today_plan' | 'visit' | 'medication' | 'reminiscence'
  order: number
  stage: CheckinStage
...
export function buildDailyCheckinScript(language: LanguageKey, plan: DailyConversationPlan): CheckinQuestion[] {
  ...
  for (let turn = 1; turn <= 12; turn += 1) {
    ...
    if (turn <= BASE_QUESTION_IDS.length) {
      ...
      script.push({
        questionId: BASE_QUESTION_IDS[turn - 1],
        responseTopic: turn === 1 ? 'dinner' : turn === 2 ? 'sleep' : turn === 3 ? 'today_plan' : 'visit',
        order: turn, stage, cognitiveSignals: STAGE_SIGNALS[stage],
    ...
    if (isMedication) {
      script.push({
        questionId: 'medication', responseTopic: 'medication', order: turn, stage: 'medication_reminder', cognitiveSignals: STAGE_SIGNALS.medication_reminder,
      ...
      script.push({
        questionId: 'reminiscence', responseTopic: 'reminiscence', order: turn, stage: 'reminiscence', cognitiveSignals: STAGE_SIGNALS.reminiscence,
        prompt, required: false, maxReprompts: DEFAULTS.maxReprompts, timeoutMs: DEFAULTS.timeoutMs,
        skippable: true, minWordsToAnswer: DEFAULTS.minWordsToAnswer,
      })
    }
  ...
  return {
    ...
    recordAnswer(text: string): 'answered' | 'insufficient' {
      ...
      if (wordCount(text) >= question.minWordsToAnswer) {
        ...
        index += 1
        // A patient often answers two adjacent prompts in one natural sentence (for example,
        // "I slept well and I am going to see Mei today"). Mark only the future prompts whose
        // semantic topic is present so the controller does not repeat itself, while retaining the
        // deterministic order for anything that was not actually mentioned.
        const coveredTopics = responseTopicsIn(text)
        for (let future = index; future < script.length; future += 1) {
          if (states[future] === 'pending' && coveredTopics.has(script[future].responseTopic)) states[future] = 'answered'
        }
        advance()
        return 'answered'
      }
...
function responseTopicsIn(text: string): Set<CheckinQuestion['responseTopic']> {
  const normalized = text.toLowerCase()
  const topics = new Set<CheckinQuestion['responseTopic']>()
  if (/(dinner|breakfast|lunch|supper|ate|eaten|food|meal|rice|noodle|porridge|吃|食|餐|饭|飯)/i.test(normalized)) topics.add('dinner')
  if (/(sleep|slept|rested|rest well|night|tired|眠|睡|休息)/i.test(normalized)) topics.add('sleep')
  if (/(today|plan|planning|going to|later|this morning|this afternoon|tomorrow|今天|打算|计划|計劃)/i.test(normalized)) topics.add('today_plan')
  if (/(visit|visiting|coming|come over|see mei|see my|family|friend|探|来看|來看|拜访|拜訪)/i.test(normalized)) topics.add('visit')
  if (/(medication|medicine|pill|pills|tablet|tablets|take it|taken it|服药|吃药|食藥|藥)/i.test(normalized)) topics.add('medication')
  if (/(holiday|vacation|travel|trip|childhood|garden|memory|remember|小时候|小時候|假期|旅行|回忆|回憶)/i.test(normalized)) topics.add('reminiscence')
  return topics
}
