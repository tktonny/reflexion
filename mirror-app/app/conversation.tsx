...
import { getDeviceFamilyMessages, type MirrorFamilyMessage } from '../src/api/familyMessages'
...
import { MirrorExperience, type MirrorHomeMessage, type MirrorHomeStatus, type MirrorHomeWidget, type MirrorVisualState } from '../src/components/mirror/MirrorExperience'
...
import { dbg } from '../src/debug/debugOverlay'
...
// Show the raw failure reason on the error screen in debug builds only.
const DEBUG_REASON_ON_SCREEN = process.env.EXPO_PUBLIC_DEBUG_ERRORS === 'on'
...
export default function ConversationScreen() {
  ...
  const [homeStatus, setHomeStatus] = useState<MirrorHomeStatus>('ready')
  ...
  const [productControl, setProductControl] = useState<'active' | 'paused'>('active')
  const [homeMessage, setHomeMessage] = useState<MirrorHomeMessage & { messageId: string; mediaUrl?: string | null; caption?: string | null } | null>(null)
  ...
  useEffect(() => {
    console.info(
      `[mirror-state] visual=${visualState} connecting=${connecting} active=${sessionActive} ` +
        `turn=${turnState || 'none'} status=${statusKind} checking=${checkingPairing}`,
    )
  }, [checkingPairing, connecting, sessionActive, statusKind, turnState, visualState])
  ...
  useEffect(() => {
    ...
    async function verifyAndLoadProfile() {
      ...
      setProductControl(productAccessForConfiguration(pairing.configuration))
      ...
      const configuredWakeTime = pairing.configuration?.patient
        ? (pairing.configuration.patient.carePlan?.dailyRoutine as { usualWakeTime?: unknown } | undefined)?.usualWakeTime
        : null
      const effectiveWakeTime = typeof configuredWakeTime === 'string' && /^\d{1,2}:\d{2}$/.test(configuredWakeTime.trim())
        ? configuredWakeTime.trim()
        : null
      const completed = await isCheckinDoneToday(wakeHourFrom(effectiveWakeTime))
      if (mounted) setHomeStatus(!pairing.online
        ? 'offline'
        : productAccessForConfiguration(pairing.configuration) === 'paused'
          ? 'paused'
          : pairing.configuration?.away?.active
            ? 'away'
  ...
  // Caregiver consent/control and Away changes are authoritative server state. Refresh the compact
  // configuration while the Mirror is idle so a pause, withdrawal, or Away period takes effect without
  // requiring a restart. A missing assignment is handled like the launch-time check and returns to setup.
  useEffect(() => {
    if (checkingPairing || !patientId) return
    let mounted = true
    const sync = async () => {
      ...
      try {
        ...
        if (!mounted) return
        const access = productAccessForConfiguration(configuration)
        setProductControl(access)
        const completed = await isCheckinDoneToday(wakeHourFrom(usualWakeTime))
        setHomeStatus(access === 'paused' ? 'paused' : configuration.away?.active ? 'away' : completed ? 'complete' : 'ready')
      } catch {
        // Heartbeat owns the offline state; a transient configuration refresh failure must not erase
        // the last known caregiver settings or interrupt an active conversation.
      }
    }
    void sync()
    const timer = setInterval(() => { if (!busy) void sync() }, 60_000)
    return () => { mounted = false; clearInterval(timer) }
  }, [busy, checkingPairing, patientId, router, usualWakeTime])
  ...
  useEffect(() => {
    ...
    // Skip buffering to speed up the ending flow.
    if (closingStage === 'idle') setClosingStage('goodbye')
  ...
  // Family messages are caregiver-authored server data. Poll only while the mirror is idle; opening a
  // message records Viewed/Played/Replayed through the canonical message route, and dismissing it does
  // not erase that interaction state.
  useEffect(() => {
    if (!patientId) return
    let mounted = true
    const load = async () => {
      const credential = await getDeviceCredential()
      if (!credential) return
      const messages = await getDeviceFamilyMessages(credential.deviceId).catch(() => [] as MirrorFamilyMessage[])
      if (!mounted || !messages.length) return
      const message = messages.find((candidate) => !candidate.interactionState) || messages[0]
      setHomeMessage({
        messageId: message.messageId,
        sender: message.senderName,
        preview: message.kind === 'voice' ? 'A voice message is waiting for you.' : (message.text || message.caption || 'A family message is waiting for you.'),
        kind: message.kind,
  ...
  useEffect(() => {
    ...
    const timerId = setInterval(() => {
      if (busy || checkingPairing || localProblem || productControl === 'paused' || endingConversation || visualState !== 'ambient') return
    ...
    return () => clearInterval(timerId)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [usualWakeTime, busy, checkingPairing, localProblem, productControl, endingConversation, visualState])
  ...
  function startWith(nextPersona: 'screening' | 'companion') {
    if (busy || checkingPairing || productControl === 'paused' || preparingStartRef.current) return
  ...
  const finalize = useCallback(async () => {
    ...
    try {
      ...
      void (async () => {
        ...
        } catch { /* save optional — already navigated away */ }
      })()
    } catch (error) {
  ...
  async function retryProblem() {
    ...
    setLocalProblem(null)
    // Actually retry the failed operation.
    void handleStart()
  }
  ...
  return (
    <View style={styles.root}>
      <MirrorExperience
        ...
        onOpenConsent={() => router.push('/consent')}
        onOpenResearch={() => router.push('/research')}
        onOpenMessage={homeMessage ? () => router.push({
          pathname: '/family-message',
          params: {
            messageId: homeMessage.messageId,
            kind: homeMessage.kind,
            sender: homeMessage.sender,
            body: homeMessage.preview,
            caption: homeMessage.caption || undefined,
            mediaUri: homeMessage.mediaUrl || undefined,
          },
        }) : undefined}
        onOpenRoutine={dailyPlan.medicationReminder ? () => router.push({
          pathname: '/routine',
          params: {
            occurrenceId: dailyPlan.medicationReminder?.occurrenceId,
            displayText: dailyPlan.medicationReminder?.displayText,
            scheduledAt: dailyPlan.medicationReminder?.scheduledAt,
          },
        }) : undefined}
...
function productAccessForConfiguration(configuration?: DeviceConfiguration): 'active' | 'paused' {
  // Older configuration bundles do not carry consent yet; preserve their existing active behaviour
  // until the server begins returning the authoritative consent state. Once present, only explicit
  // Accepted + Active permits ordinary Mirror conversations.
  const product = configuration?.consent?.product
  const control = configuration?.consent?.control
  if (control === 'paused' || (product && product !== 'accepted')) return 'paused'
  return 'active'
}
