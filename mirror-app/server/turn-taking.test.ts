...
import { createEnergyVad, decodeBase64Pcm16 } from '../src/orchestration/energyVad'
import {
...
test('daily conversation follows warm-up, yesterday recall, planning, then closes', () => {
  ...
  assert.match(screeningQuestionForTurn('english', 1, plan) ?? '', /dinner last night/i)
  assert.match(screeningQuestionForTurn('english', 2, plan) ?? '', /sleep.*rest well/i)
  assert.match(screeningQuestionForTurn('english', 3, plan) ?? '', /thinking of doing today/i)
  assert.match(screeningQuestionForTurn('english', 4, plan) ?? '', /coming to see you this week/i)
...
test('wake-word fallback feeds decoded PCM16 frames into the energy VAD', () => {
  const vad = createEnergyVad({ minSpeechMs: 100 })
  const source = new Int16Array(1600).fill(3000)
  ...
  assert.equal(vad.feed(decoded).event, 'speech_started')
})
