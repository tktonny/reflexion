...
class S3CompatibleObjectStore implements ObjectStore {
  ...
  async prepareDownload(input: { objectKey: string; contentType: string; expiresInSeconds?: number }): Promise<DownloadPlan> {
    const expiresInSeconds = Math.min(Math.max(input.expiresInSeconds || 900, 60), 3600)
    return {
      downloadUrl: this.presign('GET', input.objectKey, expiresInSeconds, {}),
      expiresAt: new Date(Date.now() + expiresInSeconds * 1000),
    }
  }
  ...
  private presign(method: 'GET' | 'PUT' | 'HEAD' | 'DELETE', objectKey: string, expires: number, headers: Record<string, string>) {
    const now = new Date()
