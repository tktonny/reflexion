...
export class LocalObjectStore implements ObjectStore {
  ...
  async prepareDownload(input: { objectKey: string; contentType: string; expiresInSeconds?: number }) {
    const { baseUrl, secret } = config()
    const expiresInSeconds = Math.min(Math.max(input.expiresInSeconds || 900, 60), 3600)
    const expiresAt = new Date(Date.now() + expiresInSeconds * 1000)
    const signature = sign(secret, input.objectKey, expiresAt.getTime())
    const query = new URLSearchParams({ key: input.objectKey, exp: String(expiresAt.getTime()), sig: signature, contentType: input.contentType })
    return { downloadUrl: `${baseUrl}/api/v1/object-store/objects?${query.toString()}`, expiresAt }
  }
  ...
  app.get('/api/v1/object-store/objects', (request, response) => {
    void (async () => {
      try {
        ...
        if (!safeEqualHex(sig, sign(secret, objectKey, exp))) return response.status(403).json({ error: 'DOWNLOAD_SIGNATURE_INVALID' })
        const body = await readFile(objectPath(dir, objectKey))
        response.setHeader('Content-Type', String(request.query.contentType || 'application/octet-stream'))
        return response.status(200).send(body)
      } catch (error) {
        if ((error as NodeJS.ErrnoException)?.code === 'ENOENT') return response.status(404).json({ error: 'OBJECT_NOT_FOUND' })
        const status = error instanceof ApiError ? error.status : 500
