  ...
  try {
    ...
    await t.test('care plan, medication, reminder and caregiver task routes', async () => {
      ...
      const familyMessage = await app.post(`/api/v1/patients/${PATIENT_ID}/family-messages`)
        ...
        .expect(201)
      assert.equal(familyMessage.body.data.deliveryState, 'queued')
      const futureMessage = await app.post(`/api/v1/patients/${PATIENT_ID}/family-messages`)
        ...
        .expect(201)
      assert.equal(futureMessage.body.data.deliveryState, 'scheduled')
      const deviceInbox = await app.get(`/api/v1/devices/${DEVICE_ID}/family-messages`).set(bearer(device)).expect(200)
      assert.equal(deviceInbox.body.data.length, 1)
      assert.equal(deviceInbox.body.data[0].deliveryState, 'delivered')
      const opened = await app.post(`/api/v1/devices/${DEVICE_ID}/family-messages/${familyMessage.body.data.messageId}/interactions`)
        .set({ ...bearer(device), 'Idempotency-Key': idempotencyKey('family-message-viewed') })
        .send({ interaction: 'viewed' })
        .expect(200)
      assert.equal(opened.body.data.interactionState, 'viewed')
      const caregiverInbox = await app.get(`/api/v1/patients/${PATIENT_ID}/family-messages`).set(bearer(human)).expect(200)
      assert.equal(caregiverInbox.body.data.find((item: any) => item.messageId === familyMessage.body.data.messageId).interactionState, 'viewed')
