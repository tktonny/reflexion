...
| 3 | 统一后端与数据底座 | 将 Mirror 路由迁入已部署的 caregiver-server/统一 API 域；统一身份、设备凭据、Session API、数据库和异步处理 | Mirror/Caregiver 使用同一认证和 API 域；密钥不落客户端；幂等 session 上传 | 未开始 |
...
- 以已部署的 caregiver-server 为起点迁入 mirror API，或建立统一 API Gateway；统一 `/api` 前缀和认证契约；
...
- 将 `platform_April` feature/identity/longitudinal 能力接到统一 session store；
